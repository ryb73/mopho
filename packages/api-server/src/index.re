...
module NapsterApi =
  NapsterApi.Make(
    {
      let apiKey = config.napster.apiKey;
    }
  );
...
let getUserIdFromNapsterMember = ({NapsterApi.id, realName}) =>
  ...
  |> then_(
       (userId) =>
         switch userId {
         | None => Db.User.create(realName) |> tap @@ flip(Db.User.setNapsterId, id)
...
let () = {
  ...
  let loginWithToken = (req, body) => {
    ...
    |> then_ @@
    saveNapsterTokens(req, body.access_token, body.refresh_token)
    |> then_ @@
    Db.User.generateAuthCode(getIp(req))
  };
  ...
  let requestAccessTokens = (req, code) => {
    ...
    |> map @@
    RespParser.parse(napsterApiAccessToken__from_json)
    |> unwrapResult
    |> then_ @@
    loginWithToken(req)
    |> map @@
  ...
  handle(
    app,
    (req, _, _, {Apis.NapsterAuth_impl.code, state}) => {
      let session = getSession(req);
...
Apis.LogInWithCode.handle(
  ...
  (req, resp, _, code) =>
    ...
    |> map(
         (token) => {
           let expires =
             momentUtc() |> Moment.add(~duration=duration(55, `years)) |> MomentRe.Moment.toDate;
           let opts =
             Response.cookieOpts(
               ~expires,
               ~httpOnly=true,
