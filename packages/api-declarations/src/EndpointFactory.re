...
module type Definition = {type req; type resp; let path: string; let reqMethod: allowedMethods;};
...
module Endpoint = (Definition: Definition) => {
  ...
  let _parseGetReq = (req) =>
    req
    |> Request.query
    |> flip(Js.Dict.get, "json")
    |? Js.Json.null
    |> Js.Json.decodeString
    |> flip(Option.bind) @@
    _tryOpt(Js.Json.parseExn)
    |? Js.Json.null
  ...
  let request = (apiUrlBase, data) => {
    ...
    |> map @@
    RespParser.parse(Definition.resp__from_json)
    |> unwrapResult
  };
};
