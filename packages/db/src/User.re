...
let create = (name: string) => {
  open Insert;
  let query = Insert.make() |> into("users") |> set("name", name) |> toString;
  doQuery(query)
  |> then_(
       ((result, _)) =>
...
let _testAffectedRows = (~expected=1, (result, _)) => {
  let affectedRows =
    Js.Json.decodeObject(result) >>= flip(Js.Dict.get, "affectedRows") >>= Js.Json.decodeNumber;
  switch affectedRows {
  | Some(actual) =>
...
let setNapsterRefreshToken = (userId: int, refreshToken: string) => {
  ...
  let query =
    ...
    |> where("userId = ?" |?. userId)
    |> toString;
  doQuery(query) |> map(_testAffectedRows)
};

let _generateAndHash = (salt) =>
  Std.generateRandomBase64()
  |> then_((code) => Std.secureHash(salt, code) |> map @@ ((hash) => (code, hash)));
    ...
    |> then_(
         ((code, hash)) => {
           let query =
             ...
             |> toString;
           doQuery(query) |> thenResolve(code)
         }
       )
  );
...
let _deleteHash = (hash: string) => {
  open Delete;
  let query =
    Delete.make() |> from("auth_codes") |> where("authCodeHash = ?" |?. hash) |> toString;
  doQuery(query)
};
